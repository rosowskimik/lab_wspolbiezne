...
void semaphore_wait_n(semaphore_t *s, uint32_t n)
{
	...
	do {
		counter =
			atomic_load_explicit(&s->counter, memory_order_acquire);
		if (counter < n) {
			futex((uint32_t *)&s->counter, FUTEX_WAIT, counter,
			      NULL, NULL, 0);
			continue;
		}
...
#error "HAULER_CNT is smaller than MAX_PER_SHIP - some ships won't be able to enter"
#endif
...
#error "PORT_SIZE is smaller than MAX_PER_SHIP - some ships won't be able to enter"
#endif
